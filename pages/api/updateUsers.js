...
handler.get(async (req, res) => {
  ...
  //let docCount = await req.db.collection('users').find({ CEDULA: { $type: "number" } }).toArray()
  let doc = await req.db.collection('users').find({ CORREO: { $type: "string" } }).toArray()
  ...
  return res.json({ errorDoc: errorDoc, newDoc: newDoc })
